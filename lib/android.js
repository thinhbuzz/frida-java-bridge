...
function _getApi () {
  ...
  temporaryApi.isApiLevel34PlusOrApexEquivalent = isArt && (
...
function _getArtRuntimeSpec (api) {
  ...
  const { isApiLevel34PlusOrApexEquivalent } = api;
  ...
  for (let offset = startOffset; offset !== endOffset; offset += pointerSize) {
    ...
    if (value.equals(vm)) {
      ...
      if (apiLevel >= 33 || codename === 'Tiramisu' || isApiLevel34PlusOrApexEquivalent) {
        // API 33: Tiramisu
        // API 34: UpsideDownCake
        // API 35: VanillaIceCream
        // API 36: Baklava
      ...
      for (const classLinkerOffset of classLinkerOffsets) {
        ...
        let heapOffset;
        if (isApiLevel34PlusOrApexEquivalent) {
...
function parseArmInstrumentationPointer (insn) {
  if (insn.mnemonic !== 'ldr' && insn.mnemonic !== 'ldr.w') {
    return null;
  }
...
const deoptimizationEnabledOffsetParsers = {
  ia32: parsex86DeoptimizationEnabledOffset,
  x64: parsex86DeoptimizationEnabledOffset,
  arm: parseArmDeoptimizationEnabledOffset,
  arm64: parseArm64DeoptimizationEnabledOffset
};

function tryDetectDeoptimizationEnabledOffset(api) {
  const impl = api['art::Instrumentation::EnableDeoptimization'];
  if (impl === undefined) {
    return null;
  }

  return parseInstructionsAt(impl, deoptimizationEnabledOffsetParsers[Process.arch], { limit: 20 });
}

function parsex86DeoptimizationEnabledOffset(insn) {
  if (insn.mnemonic !== 'mov') {
    return null;
  }
  ...
  const src = ops[1];
  if (src.type !== 'imm' || src.value.valueOf() !== 1) {
    return null;
  }

  const mem = dst.value;
  if (mem.base !== 'ecx' && mem.base !== 'rdi') {
    return null;
  }
...
function parseArmDeoptimizationEnabledOffset(insn, prevInsn) {
  ...
  const src = prevInsn.operands[1];
  if (prevInsn.mnemonic !== 'movs' || src.type !== 'imm' || src.value.valueOf() !== 1) {
    return null;
  }

  return dst.value.disp;
}

function parseArm64DeoptimizationEnabledOffset(insn, prevInsn) {
  if (insn.mnemonic !== 'strb') {
    return null;
  }
  ...
  const src = prevInsn.operands[1];
  if (prevInsn.mnemonic !== 'mov' || src.type !== 'imm' || src.value.valueOf() !== 1) {
    return null;
  }
...
function _getArtInstrumentationSpec () {
  ...
  let deoptEnabledOffset = deoptimizationEnabledOffsets[`${pointerSize}-${getAndroidApiLevel()}`];
  if (deoptEnabledOffset === undefined) {
    deoptEnabledOffset = tryDetectDeoptimizationEnabledOffset(getApi());
  }

  if (deoptEnabledOffset === undefined || deoptEnabledOffset === null) {
...
export function getArtClassSpec (vm) {
  ...
  vm.perform(env => {
    ...
    try {
      ...
      // Pair-based scan: in AOSP mirror::Class, copied_methods_offset_ (uint16)
      // is immediately followed by virtual_methods_offset_ (uint16).
      // We constrain both values using known direct/virtual method indices
      // so detection works even if Thread gains copied methods in the future.
      const methodDirect = unwrapMethodId(env.getStaticMethodId(clazzRef, 'currentThread', '()Ljava/lang/Thread;'));
      const directIdx = methodDirect.sub(methodsArray.data).toNumber() / methodSpec.size;
      const virtualIdx = methodInstance.sub(methodsArray.data).toNumber() / methodSpec.size;

      for (let offset = offsetMethods + 4; offset !== MAX_OFFSET; offset += 4) {
        const cmo = object.add(offset).readU16();
        const vmo = object.add(offset + 2).readU16();
        if (vmo > directIdx && vmo <= virtualIdx && cmo >= vmo && cmo <= methodsArraySize) {
          offsetCopiedMethods = offset;
          break;
        }
      }

      // Fallback: legacy detection matching total method count
...
function _getArtThreadSpec (vm) {
  ...
  vm.perform(env => {
    ...
    for (let offset = 128; offset !== 384; offset += pointerSize) {
...
function _getArtApexVersion () {
  try {
    ...
    for (const line of mountInfo.trimEnd().split('\n')) {
      ...
      const dashIndex = elements.indexOf('-');
      if (dashIndex === -1 || dashIndex + 1 >= elements.length) {
        continue;
      }
      const mountSource = elements[dashIndex + 2];
...
function instrumentArtMethodInvocationFromInterpreter () {
  ...
  } else if (!isApiLevel34PlusOrApexEquivalent && apiLevel <= 33) {
...
function ensureArtKnowsHowToHandleReplacementMethods (vm) {
  ...
  if (copyingPhase !== null) {
    ...
    const art = api.module;
    let markCompactPhase = art.findSymbolByName('_ZN3art2gc9collector11MarkCompact12MarkingPhaseEv');
    if (markCompactPhase === null) {
      markCompactPhase = art.findSymbolByName('_ZN3art2gc9collector11MarkCompact14CompactionPhaseEv');
    }
    if (markCompactPhase !== null) {
      Interceptor.attach(markCompactPhase, artController.hooks.Gc.copyingPhase);
    }
  }
...
let cachedJniIdOffsets = null;

function unwrapGenericId (genericId, apiMethod) {
  const api = getApi();

  if (cachedJniIdOffsets === null) {
    const runtimeOffset = getArtRuntimeSpec(api).offset;
    cachedJniIdOffsets = {
      jniIdManager: runtimeOffset.jniIdManager,
      jniIdsIndirection: runtimeOffset.jniIdsIndirection
    };
  }

  const { jniIdManager: jniIdManagerOffset, jniIdsIndirection: jniIdsIndirectionOffset } = cachedJniIdOffsets;
